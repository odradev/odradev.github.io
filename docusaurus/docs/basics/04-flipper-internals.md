...
mod tests {
    ...
    use odra::host::{Deployer, DefaultInitArgs};
    ...
    fn flipping() {
        ...
        let mut contract = FlipperHostRef::deploy(&env, DefaultInitArgs);
        assert!(!contract.get());
        contract.flip();
        assert!(contract.get());
    }
...
as the second argument of the deploy function, we pass `DefaultInitArgs`, one of the implementations of 
