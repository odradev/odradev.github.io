...
First, `cargo odra` runs the `wasm-opt` tool:

```bash
wasm-opt --signext-lowering wasm/my_contract.wasm -o wasm/my_contract.wasm
```

Then it lowers the size of the wasm file with the `wasm-strip` tool:
```bash
wasm-strip wasm/my_contract.wasm
```

:::caution
The order matters. `wasm-strip` removes the sections `wasm-opt` needs to validate the module, so
running it first makes `wasm-opt` fail with thousands of validation errors.
:::
